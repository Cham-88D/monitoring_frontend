...
export const PosB = () => {
  ...
  const getData = async () => {
    ...
    try {
      const response = await axios.post("/api/pos/pos_bar", {
        email: email,
      });
                ...
                >
                  {nodata ? (
                    <BarChart width={600} height={400} data={processedData}>
                      ...
                      <Bar dataKey="correct" fill="#F99C30" barSize={20} />
                      <Bar dataKey="incorrect" fill="#FCD7AC" barSize={20} />
                    </BarChart>
                  ) : (
                    <Box
                      sx={{
